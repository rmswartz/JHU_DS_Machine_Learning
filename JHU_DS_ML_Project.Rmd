...
_**Step 3**_: After inspection of the training data with the `str()` function, clean the data (both training and test sets) by removing the predictors with 'NA' values among their observations and subsetting the data to the outcome and the measurements from the accelerometers. Also, convert any measurements inadvertently stored as `factor` to `numeric`.
...
_**Step 4**_: Split the original training data into two sets based on a 75% random sub-sampling scheme to enable cross-validation of the model options and insight to select the best approach before the final test on the `classe` variable. `classe` indicates correct or incorrect execution of the exercise with codes A through E, where A indicates a correct execution, while the others classify a variety of improper executions. For full details, please see Reference 1. 
...
_**Step 5**_: In this portion of the analysis, the researcher selects two sets of predictors for the possible models for evaluation of the better method. by removal of those with near zero covariance to limit the predictors to just those that are likely to be good predictors.

_Set 1_: Removing Near Zero Covariates (NZC) identifies those predictors with near zero covariance to limit the predictors to just those that are likely to be good.
...
_Set 2_: To select an alternative set of predictors, Principal Components Analysis (PCA) is used to collapse the clean training data's predictors to weighted combinations. This set will be used to train a variety of models and compare the performance of those models against that of models trained with the clean training set, allowing the researcher to determine which method of predictor selection is best for this problem.
...
As it turns out, this does not remove any predictors from consideration as compared to the training set; therefore, all predictors in the training data are sufficiently variable in comparison to each other. 

#Model Construction and Error Measurement

_**Model Selection**_
With two possible sets of variables to consider (all those in the clean training set and those resulting from a PCA), the researcher created four models (Bagging and Random Forest approach for each variable set) and evaluated their performance based on accuracy and kappa. Based upon this, the researcher built a final model using the predictor selection method and modeling approach that yielded the best performance with the entire training set.
